...
describe('toMarkdown', () => {
	...
	it('uses dash bullets for list items', () => {
		const result = toMarkdown({ standup: baseStandup });
	...
	it('normalizes existing bullets to dashes', () => {
		const input: FormatterInput = {
			standup: {
				...baseStandup,
				workCompleted: '• Bullet points\n* Asterisks\n- Already dashes',
			},
		};
		...
		expect(result).toContain('- Already dashes');
	...
	it('formats linked issues as markdown links', () => {
		const input: FormatterInput = {
			standup: {
				...baseStandup,
				linkedTasks: [
					...
					},
					{
						...
						externalLinks: [{
							externalId: '#48',
							source: 'github',
							externalUrl: 'https://github.com/org/repo/issues/48',
							confidence: 'explicit',
						}],
						createdAt: new Date(),
					},
		...
		expect(result).toContain('[#48](https://github.com/org/repo/issues/48)');
	});
